...
def upfirdn2d_native(
    ...
    tensor = tensor.reshape(-1, in_h, in_w, 1) # [B*C, H, W, 1]

    _, in_h, in_w, minor = tensor.shape
    kernel_h, kernel_w = kernel.shape

    out = tensor.view(-1, in_h, 1, in_w, 1, minor) # [B*C, H, 1, W, 1, 1]
    out = F.pad(out, [0, 0, 0, up_x - 1, 0, 0, 0, up_y - 1]) # [B*C, H, uy, W, ux, 1]
    out = out.view(-1, in_h * up_y, in_w * up_x, minor) # [B*C, uyH, uxW, 1]
    ...
    out = out.permute(0, 3, 1, 2) # [B*C, uyH(padded), uxW(padded), 1]
    out = out.reshape([-1, 1, in_h * up_y + pad_y0 + pad_y1, in_w * up_x + pad_x0 + pad_x1]) # [B*C, 1, uyH(padded), uxW(padded)]
    w = torch.flip(kernel, [0, 1]).view(1, 1, kernel_h, kernel_w) # [1, 1, Kh, Kw]
    out = F.conv2d(out, w) # actually cross-correlation, so flip the kernel first
    out = out.reshape(
        -1,
        minor,
        in_h * up_y + pad_y0 + pad_y1 - kernel_h + 1,
        in_w * up_x + pad_x0 + pad_x1 - kernel_w + 1,
    )  # [B*C, 1, uyH(conved), uxW(conved)]
    out = out.permute(0, 2, 3, 1) # [B*C, uyH(conved), uxW(conved), 1]
    out = out[:, ::down_y, ::down_x, :] # [B*C, out_h, out_w, 1]
...
def upsample_2d(
    ...
    Accepts a batch of 2D images of the shape `[N, C, H, W]` and upsamples each image with the given
